...
import { Camera, Bot, ArrowRight, Loader2, CheckCircle2, Video, FileText, Image as ImageIcon } from 'lucide-react'
...
type Step2Phase = 
  ...
  | 'script-generating'
  | 'script-editing'
  | 'shooting-guide' // manual only
  | 'video-upload' // manual only
  | 'image-selection' // auto only
  | 'video-creating'
...
export default function Step2Page() {
  ...
  const [phase, setPhase] = useState<Step2Phase>('mode-selection')
  ...
  const handleModeSelect = (selectedMode: Step2Mode) => {
    ...
    setPhase('script-style-selection')
  }
  ...
  const handleConfirmScript = () => {
    if (mode === 'manual') {
      setPhase('shooting-guide')
    } else {
      setPhase('image-selection')
    }
  }

  // 촬영 안내 후 업로드로 이동
  const handleGoToUpload = () => {
    setPhase('video-upload')
  }
  ...
  // 대본 스타일 선택 화면
  if (phase === 'script-style-selection') {
    return (
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
  ...
  // 촬영 안내 화면 (manual only)
  if (phase === 'shooting-guide') {
    return (
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
  ...
  // 영상 업로드 화면 (manual only)
  if (phase === 'video-upload') {
    return (
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: 1, x: 0 }}
